...
def get_config_file_path_from_name_or_path(experiment_name_or_path: str, similarity_threshold: int) -> str:
    if not experiment_name_or_path.endswith(".jsonnet"):
        ...
        if similarity_threshold is not None:
            experiment_name_or_path = experiment_name_or_path + "_similarity_" + str(similarity_threshold)
        ...
        if len(matching_result) != 1:
            exit(f"Couldn't find one matching path with the given name ({experiment_name_or_path}) or found than 1 file.")
...
def find_matching_paragraph_text(corpus_name: str, original_paragraph_text: str) -> str:
    ...
    result = requests.post(url, json=params, headers={'Connection':'close'})
